...
# Visualization - Modified layout
plt.figure(figsize=(20, 15))

# Confusion Matrix - Top row, 2x wider
plt.subplot(2, 2, (1, 2))  # This spans columns 1 and 2 of the first row
cm = confusion_matrix(y_test, y_pred)
sns.heatmap(cm, annot=False, cmap='Blues',
            xticklabels=label_encoder.classes_,
            yticklabels=label_encoder.classes_)
plt.title('Confusion Matrix - Which genres get confused?', fontsize=14, fontweight='bold')
plt.xticks(rotation=45)
plt.yticks(rotation=0)
plt.xlabel('Predicted Genre', fontsize=12)
plt.ylabel('Actual Genre', fontsize=12)

# Feature Importance Plot - Bottom left
plt.subplot(2, 2, 3)
sns.barplot(data=feature_importance, x='importance', y='feature')
plt.title('Top Audio Features for Genre Prediction', fontsize=12, fontweight='bold')
plt.xlabel('Importance (%)', fontsize=10)

# Genre Performance Chart - Bottom right
plt.subplot(2, 2, 4)
...
plt.title('Top 10 Genres by Prediction Accuracy', fontsize=12, fontweight='bold')
plt.xlabel('F1-Score (Higher is Better)', fontsize=10)
